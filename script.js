const baseUrl = "https://github-repo-okeibunor.netlify.app/.netlify/functions/getData";
...
fetch(baseUrl, {
  ...
  let repoArray = res.data.data.user.repositories.nodes;
  let output = '';
  ...
  document.querySelector('.profile_sum').innerHTML = `
    <div class="user-details">
      <div class="name">${res.data.data.user.name}</div>
      <p class="login">${res.data.data.user.login}</p>
      <div class="bio">${res.data.data.user.bio}</div>
    </div>

    `
  document.querySelector('.avatar-container').innerHTML = `
      <img class="avatar" src=${res.data.data.user.avatarUrl} alt="avatar" />
...
var header = document.querySelector(".bottom-nav");
var navHeight = header.offsetTop;

var avatarCompressed = document.querySelector(".avatar-compressed");
var lorem = document.querySelector(".avatar")
var avatarHeight = lorem.offsetTop + 100;
...
// fade in
function showAvatar() {
  if (window.pageYOffset > avatarHeight) {
    avatarCompressed.classList.add("show");
  } else {
    avatarCompressed.classList.remove("show");
  }
}